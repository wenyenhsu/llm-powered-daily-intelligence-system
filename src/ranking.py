...
def build_ranking():
    ...
    # 1) Scan the entire library to count how many times each insight is mentioned
    for md in BASE_DIR.rglob("*.md"):
        ...
        for link in links:
            ...
            # If it's not the current file, consider it a backlink
            if md.parent.name != "insights" or md.stem != link:
                backlinks[link].add(str(md))

    # 2) Weight recent daily files by the last 7 days
    ...
    for insight in set(list(mentions.keys()) + list(backlinks.keys()) + list(recent_mentions.keys())):
        ...
        # Backlink from other files
        score += len(backlinks[insight]) * 5

        # Recent frequency
        score += recent_mentions[insight] * 3

        # Canonical insight gets an extra point if the file exists
